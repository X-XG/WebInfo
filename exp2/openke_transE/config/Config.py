...
class Config(object):
    ...
    def predict_tail(self,h,r,k):
        res = []
        from tqdm import tqdm
        for i in tqdm(range(len(h))):
            test_h = np.array([int(h[i])]*self.entTotal)
            test_r = np.array([int(r[i])]*self.entTotal)
            test_t = np.array(range(self.entTotal))
            res.append(self.test_one_step(self.testModel,test_h,test_t,test_r).argsort()[:k])
        return res
